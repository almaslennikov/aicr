...
`,
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			// (AICR_MAX_NODES_PER_ENTRY). The network collector's flags have
			// cli.EnvVars Sources, so opts.clusterConfigPath /
			// opts.discoverNetwork are already populated from the env when
			// the Job set them — but we also seed the env vars from opts in
			// the dev-bypass case (developer runs `AICR_AGENT_MODE=true aicr
			// snapshot --cluster-config <path>` from their machine to skip
			// Job deployment) so the rebuild picks them up too.
			if os.Getenv("AICR_AGENT_MODE") == "true" {
				if opts.clusterConfigPath != "" {
					_ = os.Setenv("AICR_CLUSTER_CONFIG_PATH", opts.clusterConfigPath)
				}
				if opts.discoverNetwork {
					_ = os.Setenv("AICR_DISCOVER_NETWORK", "true")
				}
				// Seed KUBECONFIG so the network collector (which calls
				// l8k's kubeclient.New, which in turn calls clientcmd
				// directly) finds the same kubeconfig the rest of the
				// CLI is configured against. clientcmd's loading rules
				// honor KUBECONFIG when its explicit-path arg is empty.
